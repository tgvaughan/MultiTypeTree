...
public class ColouredSubtreeSlide extends ColouredTreeOperator {
    ...
    public double proposal() {
        ...
        if (parent.isRoot()) {
            ...
            logHR -= geRootMoveProb(node, t_parentNew,
            ...
            logHR += geRootMoveProb(node, t_parentOld,
    ...
    /**
     * Obtain probability of a particular move of branch starting at node to
     * join to its parent at time newParentTime and with nChanges colour changes
     * along it. Also handles moves in which the root node is repositioned.
     *
     * @param node
     * @param newParentTime
     * @param nChanges
     ...
     */
    double geRootMoveProb(Node node, double newParentTime,
     ...
     */
    private boolean recolourRootBranch(Node node,
            int nChangesNode, int nChangesSister) {
        ...
        } else // Force reject if colouring inconsistent:
