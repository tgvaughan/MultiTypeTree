...
public class TipTypesInputEditor extends BEASTObjectInputEditor {

    public TipTypesInputEditor(BeautiDoc doc) {
        super(doc);
    }
    ...
    public void init(Input<?> input, BEASTObject plugin, int itemNr, ExpandOption bExpandOption, boolean bAddButtons) {
        ...
        this.itemNr = itemNr;
        if (itemNr >= 0) {
            tree = (MultiTypeTree) ((List<?>) input.get()).get(itemNr);
        } else {
            tree = (MultiTypeTree) input.get();
        }
        if (tree != null) {
            try {
                    ...
                    try {
                        if (checkBox.isSelected()) {
                            if (traitSet == null) {
                                ...
                                traitSet.initByName("traitname", "type",
